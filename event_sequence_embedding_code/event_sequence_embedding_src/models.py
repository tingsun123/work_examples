...
class Word2Vec(object):
    ...
    def fit(self, sentences, size=100, alpha=0.025, window=5, min_count=5, max_vocab_size=None,
        ...
        # save as text file
        self.model.wv.save_word2vec_format(os.path.join(self.save_folder, "gensim-model.txt"), binary=False)
...
def create_embeddings(gensim_model=None, model_folder=None):
    ...
    with tf.Session() as sess:
         ...
         print(
        'Run `tensorboard --logdir={0}` to run visualize result on tensorboard'.format(model_folder))
